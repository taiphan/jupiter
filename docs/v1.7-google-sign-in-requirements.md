...
- Microsoft / GitHub / Apple social providers — [v1.11 breadth](./v2.0-auth-enterprise-breadth-requirements.md#part-a--v111-auth-breadth)
- SAML / OIDC / SCIM / org policy — [v2.0 enterprise](./v2.0-auth-enterprise-breadth-requirements.md#part-b--v20-enterprise-identity)
- Google One Tap only without full OAuth callback (optional stretch)
- Workspace domain restriction — **shipped** via `GOOGLE_ALLOWED_HD` (optional env)
...
**Login page (all auth):** See [REQUIREMENTS.md §7.1.1](../REQUIREMENTS.md#711-login--sign-up-ui-required) — `/api/auth/config` drives which of Google / Microsoft / GitHub / email appear on `/login` and `/signup`.
